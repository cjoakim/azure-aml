...
def main():
    ...
    output_blobname    = keyword_arg('--output-blobname', None)
    print('input_dataset_name: {}'.format(input_dataset_name))
    print('output_blobname:    {}'.format(output_blobname))
    ...
    output_path = 'tmp/{}'.format(output_blobname)
    lls_df.to_csv(output_path)

    # Upload the prepared csv file to Azure Storage
    BlobUtil(run=run).upload(output_path)
